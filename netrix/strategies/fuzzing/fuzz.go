...
func (f *FuzzStrategy) Step(e *types.Event, ctx *strategies.Context) {
	...
	var nextReplica types.ReplicaID

	// 3. Check if there is anything to schedule
	if choice, ok := f.mutatedNodeChoices.Pop(); ok {
		nextReplica = choice
	} else {
		r, _ := f.replicaStore.GetRandom()
		nextReplica = r.ID
	}
	mailbox, ok := f.mailBoxes.Get(nextReplica)
	if ok && len(mailbox) > 0 {
		nextMessageID := mailbox[0]
		mailbox = mailbox[1:]
		f.mailBoxes.Add(nextReplica, mailbox)

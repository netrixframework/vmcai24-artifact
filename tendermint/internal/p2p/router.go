...
func (r *Router) handshakePeer(
	...
	peerInfo, _, err := conn.Handshake(ctx, r.options.HandshakeTimeout, *nodeInfo, r.privKey)
	if err != nil {
		return peerInfo, err
	}
...
func (r *Router) sendPeer(ctx context.Context, peerID types.NodeID, conn Connection, peerQueue queue) error {
	for {
		...
		case envelope := <-peerQueue.dequeue():
			...
			r.logger.Debug("sent message", "peer", envelope.To, "message", envelope.Message, "channel_id", envelope.ChannelID)

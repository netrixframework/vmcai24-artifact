...
import (
	...
	replicaclient "github.com/netrixframework/go-clientlibrary"
...
func (cs *State) newStep() {
	...
	c, err := replicaclient.GetClient()
	if err == nil {
		c.PublishEvent("newStep", map[string]string{
			"height": strconv.Itoa(int(rs.Height)),
			"round":  strconv.Itoa(int(rs.Round)),
			"step":   rs.Step,
		})
	}
...
func (cs *State) enterNewRound(ctx context.Context, height int64, round int32) {
	...
	c, err := replicaclient.GetClient()
	if err == nil {
		c.PublishEvent("newRound", map[string]string{
...
func (cs *State) finalizeCommit(ctx context.Context, height int64) {
	...
	c, er := replicaclient.GetClient()
	if er == nil {
		c.PublishEventAsync("CommittingBlock", map[string]string{
			"block_id": blockID.Hash.String(),
			"height":   strconv.FormatInt(cs.Height, 10),
			"round":    strconv.FormatInt(int64(cs.Round), 10),
			"step":     cs.Step.String(),
		})
	}

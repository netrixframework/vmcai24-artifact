...
func MakeInitFilesCommand(conf *config.Config, logger log.Logger) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			conf.Mode = args[0]
			return initFilesWithConfig(cmd.Context(), -1, conf, logger, keyType)
		},
	}
...
func initFilesWithConfig(ctx context.Context, index int, conf *config.Config, logger log.Logger, keyType string) error {
	var (
		pv  *privval.FilePV
		err error
	)
	...
	if tmos.FileExists(nodeKeyFile) {
		...
		if _, err := types.LoadOrGenNodeKeyWithID(nodeKeyFile, strconv.Itoa(index)); err != nil {
			return err
		}
		logger.Info("Generated node key", "path", nodeKeyFile)
	}

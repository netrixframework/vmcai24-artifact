...
Example:
	...
	cmd.Flags().StringVar(&ipAddress, "ip-address", "127.0.0.1", "IP address of the node")
	...
	cmd.Flags().IntVar(&startingP2PPort, "starting-p2p-port", -1, "starting P2P port for peer address generation")
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		for i := 0; i < nValidators; i++ {
			...
			if err := initFilesWithConfig(ctx, i, config, logger, keyType); err != nil {
				return err
			}
		...
		for i := 0; i < nNonValidators; i++ {
			...
			if err := initFilesWithConfig(ctx, i+nValidators, conf, logger, keyType); err != nil {
				return err
			}
		}
		...
		tpargs := testnetPeerArgs{
			...
			startingP2PPort:  startingP2PPort,
		}
		...
		for i := 0; i < nValidators+nNonValidators; i++ {
			...
			config.RPC.ListenAddress = "tcp://127.0.0.1:2565" + strconv.Itoa(i)
			config.RPC.PprofListenAddress = "127.0.0.1:606" + strconv.Itoa(i)
			config.P2P.ListenAddress = "tcp://127.0.0.1:" + strconv.Itoa(26656+i)
			config.P2P.NetrixClientAddr = "127.0.0.1:808" + strconv.Itoa(i+1)
...
func (args *testnetPeerArgs) hostnameOrIP(i int) (string, error) {
	...
	if args.startingIPAddr == "" && args.startingP2PPort == -1 {
	...
	if args.startingIPAddr != "" {
		...
		return fmt.Sprintf("%s:%d", ip.String(), args.peerToPeerPort), nil
	}
	return fmt.Sprintf("%s:%d", args.ipAddr, args.startingP2PPort+i), nil
...
func persistentPeersArray(config *cfg.Config, args testnetPeerArgs) ([]string, error) {
	...
	for i := 0; i < len(peers); i++ {
		...
		peers[i] = nodeKey.AddressString(addr)
	}
	return peers, nil
}
